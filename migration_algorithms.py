...
class SCG:
    ...
    def reverse_offloading(
        mec_set: list, vr_users: list, user_ip: str, service_id: str
    ) -> None:
    ...
    def migration(
        ...
        if mec_candidate_location is not None:
            ...
            if new_service_latency < previous_service_latency:
                ...
                migration.sucessful_migrations += 1
            ...
            """
            migration.unsuccessful_migrations += 1

    @staticmethod
    def trade_off(
        base_station_set: list, mec_set: list, vr_users: list, migration: Migration, hosts: dict, 
    ):
        ...
        for user in vr_users:
            for service_id in user.services_ids:
                user_location = VrController.get_vr_user_location(hosts=hosts, user_ip=user.ip)
                if any(service["id"] == service_id for service in user.services_set):
                    ...
                    vr_service_content = VrController.get_vr_service(
                        vr_users=vr_users, user_ip=user.ip, service_id=service_id
                    )
